...
//
// 实现要点：
//   - 14 个子命令：discover / run / push / pull / topology / node / trust / psk /
//                  auth / migrate / config / completion / version / help
//   - 13 个退出码（ADR-010 §3.4）
//   - 6 个全局选项 + shell completion
...
int cmd_node(const std::vector<std::string>& args) {
    ...
    if (args.size() >= 5 && args[0] == "register") {
        ...
        if (r.is_err()) return kInvalidArg;
...
int cmd_auth(const std::vector<std::string>& args) {
    ...
    if (args.size() >= 2 && args[0] == "psk") {
        ...
        if (r.is_err()) return kWhitelistDenied;
        std::cout << "{\"sequence\":" << r.value() << "}" << std::endl;
        return kOk;
    }
...
const std::unordered_map<std::string, Command>& command_table() {
    static const std::unordered_map<std::string, Command> tbl = {
