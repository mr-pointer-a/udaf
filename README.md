...
当前覆盖率：**96.6% lines / 97.3% functions**（604 单元 + 9 集成 + 1 模糊 + 1 故障注入 + 1 端到端；剩余未覆盖行主要为 OpenSSL API 失败分支 / 系统调用错误路径 / lcov 工具限制）。
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.3.16 | 2026-09-02 | libudaf_fi.c 警告全部清零 + C/CXX 编译选项分流 + pki.cpp 0% → 91.0% 覆盖（+71 行 + 100% 函数）/ 616 测试全绿 |
| v0.3.15 | 2026-09-02 | F10~F19 多模块覆盖率提升 + 不可达路径分析 + libudaf_fi.so skip-first-N |
